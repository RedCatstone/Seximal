                ...
                <svg viewBox="0 0 {strokesThisGroup * TALLY_WIDTH} {TALLY_HEIGHT}">
                    {#each { length: strokesThisGroup } as _, i}
                        {#if i !== tallyGroupSize - 1}
                            <!-- vertical sticks -->
                            <line class="stick" x1={(i + 0.5) * TALLY_WIDTH} y1={TALLY_HEIGHT} x2={(i + 0.75) * TALLY_WIDTH} y2="0" />
                        {:else}
                            <!-- diagonal strike -->
                            <line class="strike" x1="0" y1={TALLY_HEIGHT * 0.75} x2={(tallyGroupSize - 0.75) * TALLY_WIDTH} y2={TALLY_HEIGHT * 0.25} />
                        {/if}
                    {/each}
                </svg>
            </div>
        {/each}
        {#if Math.ceil(Math.abs(value) / tallyGroupSize) > MAX_TALLY_MARKS_DRAWN}<span style:font-size="2.5rem">...</span>{/if}
    </div>
</div>
